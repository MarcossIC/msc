...
pub fn create_cookie_file(url: &Url, cookie_str: &str) -> Result<PathBuf> {
    ...
    for cookie_pair in cookie_str.split(';') {
        ...
        if parts.len() != 2 {
            println!(
                "{}",
                format!("⚠️  Cookie inválida (ignorada): {}", cookie_pair).yellow()
            );
            continue;
        }
    ...
    println!(
        "{}",
        format!("   ✓ Archivo de cookies creado: {}", cookie_file.display()).dimmed()
    );
...
pub fn find_browser_cookie_db(browser: &str) -> Result<PathBuf> {
    let app_data = if cfg!(windows) {
        env::var("LOCALAPPDATA").context("LOCALAPPDATA no está configurado")?
    } else if cfg!(target_os = "macos") {
        let home = env::var("HOME").context("HOME no está configurado")?;
    ...
    let cookie_path = match browser.to_lowercase().as_str() {
        ...
        "firefox" => {
            ...
            if cfg!(windows) {
                let roaming = env::var("APPDATA").context("APPDATA no está configurado")?;
    ...
    if !cookie_path.exists() {
        return Err(anyhow!(
            "No se encontró la base de datos de cookies en: {}\n\
            Asegúrate de que:\n\
            • El navegador esté instalado\n\
            • Has usado el navegador al menos una vez\n\
            • La ruta del perfil es correcta",
            cookie_path.display()
        ));
    }
...
pub fn find_firefox_cookie_db(profiles_dir: &Path) -> Result<PathBuf> {
    ...
    Err(anyhow!(
        "No se encontró 'cookies.sqlite' en ningún perfil válido dentro de: {}",
        profiles_dir.display()
    ))
}
...
pub fn extract_cookies_from_db(db_path: &Path, domain: &str) -> Result<Vec<Cookie>> {
    ...
    if wal_path.exists() && fs::copy(&wal_path, &temp_wal).is_ok() {
        println!(
            "{}",
            "   ℹ️  Navegador abierto detectado, leyendo archivo WAL...".yellow()
        );
    }
    ...
    if let Ok(mut stmt) = conn.prepare(chrome_query) {
        ...
        if let Ok(mut stmt) = conn.prepare(firefox_query) {
            ...
            if cookies.is_empty() {
                ...
                eprintln!(
                    "{}",
                    "🔍 Debug: No se encontraron cookies con el patrón de búsqueda.".yellow()
                );
                eprintln!(
                    "{}",
                    format!(
...
pub fn debug_database_info(db_path: &Path) -> Result<()> {
    println!("{}", "🔍 INFORMACIÓN DE LA BASE DE DATOS".cyan().bold());
    println!("{}", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".cyan());
    println!();
    ...
    // 1. List all tables
    println!("{}", "📊 TABLAS EN LA BASE DE DATOS:".green().bold());
    let mut stmt =
    ...
    if cookie_tables.is_empty() {
        println!(
            "{}",
            "⚠️  No se encontraron tablas relacionadas con cookies".yellow()
        );
        println!();
    } else {
        for table_name in cookie_tables {
            println!(
                "{} {}",
                "📋 ESQUEMA DE LA TABLA:".green().bold(),
                table_name.cyan()
            );
            ...
            for (_, col_name, col_type) in &columns {
                println!("   • {} ({})", col_name.cyan(), col_type.dimmed());
            }
            println!();
            ...
            if count > 0 {
                println!("{}", "   📝 MUESTRA DE DATOS (primeras 5 filas):".dimmed());
            ...
            println!("{}", "   🔍 BÚSQUEDA DE DOMINIOS POPULARES:".dimmed());
    ...
    match cookies {
        Some(cookie_input) => {
            ...
            if path.exists() && path.is_file() {
                ...
                let url_parsed = Url::parse(url).context("URL inválida para cookies")?;
