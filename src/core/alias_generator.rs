...
impl AliasGenerator for UnixScriptGenerator {
        ...
        #[cfg(unix)]
        {
            if target_path.exists() {
                ...
                let metadata = fs::symlink_metadata(&target_path)
                    .with_context(|| format!("Failed to read metadata for {:?}", target_path))?;

                if metadata.is_symlink() {
                    anyhow::bail!(
                        "Security Error: Refusing to overwrite symlink at {:?}. \
                        This could be a symlink attack attempting to overwrite sensitive files. \
                        Please remove the symlink manually if this is unintentional.",
                        target_path
                    );
                }
            }
    ...
    fn cleanup(&self, alias_name: &str, output_path: &Path) -> Result<()> {
        ...
        if target_path.exists() {
            // SECURITY: Use symlink_metadata to remove the symlink itself, not the target
            #[cfg(unix)]
            {
                let metadata = fs::symlink_metadata(&target_path)
                    .with_context(|| format!("Failed to read metadata for {:?}", target_path))?;
                ...
                if metadata.is_symlink() {
                    // Log warning but still remove
                    eprintln!(
                        "Warning: Removing symlink at {:?}. If this is unexpected, investigate for security issues.",
                        target_path
                    );
                }
            }
